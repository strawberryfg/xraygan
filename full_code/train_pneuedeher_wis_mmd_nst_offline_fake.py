...
alpha_mmd = 3
alpha_c = 1.0
alpha_nst_1 = 0.003 #0.003 first for ~3 epochs (at first around 300)
alpha_nst_style = 0.025
confidenceThresh = 0.80 #disable
train_g_more = True
train_d_more = not(train_g_more)
g_vs_d = 3
d_vs_g = 3
discriminator_fake_thresh = 0.25 # if discriminator(img) < this threshold then make sense
per_class_sample = True
weights_prefix = '../models_pneuedeher_wis_mmd_nst_offline_fake/'
weights_middle_name = "_pneuedeher_is_mmd_nst_offline_fake_allow100000_epo_"
#0. torch imports
import torch
...
fake_intermediate_folders = [ 'train_fake/', 'train_fake_allclasses_wis_mmd/', 'train_fake_allclasses_wis_mmd_nst/', 'train_fake_allclasses_wis_only/', 'train_fake_allclasses_wo/', 'train_fake_pneuede/' ]
epoch_st_folders = [ 20, 98, 98, 98, 98, 15]
epoch_en_folders = [ 123, 116, 126, 100, 105, 45]
suffix_max = [ 155, 285, 285, 285, 285, 285]
suffix = 'images/'
image_index_list = []    
labels_list = []
...
allowed_fake_img_num = 100000

fake_img_pool_folder = "fake_img_pool/"    
...
# sample fake images from folders
def sample_fake_images_randomly(fake_cnt, saved):
    inputs = []
    labels = []
    if saved:
        # just load from the disk
        for i in range(batch_size):
            img_path = root_dir + fake_img_pool_folder + str(fake_cnt) + ".png"
            ...
            inputs.append(img)
            fake_cnt += 1
            if fake_cnt > allowed_fake_img_num:
                fake_cnt %= allowed_fake_img_num
            labels.append(0) #dummy
    else:
        len_folders = len(fake_intermediate_folders)
        folder_id = random.randint(0, len_folders - 1)
        st = epoch_st_folders[folder_id]
        en = epoch_en_folders[folder_id]
        epo = random.randint(st, en - 1)
        suf = random.randint(0, suffix_max[folder_id] / 5 + 1) * 5
        original_img_path = root_dir + fake_intermediate_folders[folder_id] + str(epo) + "_" + str(suf) + ".png"
        ...
        original_img = original_img / 256.0
        original_img = original_img.astype(np.float32)
        original_img = torch.from_numpy(original_img)
        original_img = original_img.reshape((original_img.shape[0], original_img.shape[1], 3))
        original_img = original_img.permute(2, 0, 1).data.cpu().numpy()
        ...
        y = 0
        for i in range(batch_size):
            img = original_img[:, y:y + 128, x:x + 128]
            if x + 128 >= W_max:
                x = 0
                y += 128
            else: 
            ...
            #print(img.shape)
            labels.append(0) #dummy
            fake_img_save_path = root_dir + fake_img_pool_folder + str(fake_cnt) + ".png"
            ...
            img = open_cv_image.astype(np.uint8)
                            
            #print(img.shape)
            cv2.imwrite(fake_img_save_path, img)
            fake_cnt += 1
        if fake_cnt >= allowed_fake_img_num:
           saved = True
           fake_cnt = 0 # start from 0 again    
    ...
    return inputs, labels, fake_cnt, saved
...
def train(total_trained_samples):
  fake_cnt = 0  
  saved = False #already save allowed_fake_img_num number of images
  netD.train()
  netG.train()
  total_train = 0
  correct_train = 0
  ...
  for epoch in range(start_epoch, epochs):
    ...
    for steps in range(epoch_imgs // batch_size):
        ...
        fakeImageBatch, dummy, fake_cnt, saved = sample_fake_images_randomly(fake_cnt, saved)
        #print(fakeImageBatch.shape)
        predictionsFake = netD(fakeImageBatch[:,0:1,:,:])
        predictionsFake_discriminator = predictionsFake.detach().clone()
        if steps % display_per_iters == 0:
           print(predictionsFake)
           print(predictionsFake.shape)
           print("===========")
        ...
        #loss_fns_style_only = [GramMSELoss()] * len(style_layers) 
        
        #if torch.cuda.is_available():

        #   loss_fns = [loss_fn.cuda() for loss_fn in loss_fns]
           #for loss_fn in loss_fns_style_only:
           #    print(loss_fn)
           #print(loss_fns_style_only)
        #   loss_fns_style_only = [loss_fn.cuda() for loss_fn in loss_fns_style_only]
        ...
        # visualize optimized image
        # only show the first
        #opt_img = opt.data[0].cpu().numpy().transpose(1, 2, 0)
        #if steps % display_per_iters == 0:
        #   cv2.imshow("opt", opt_img)
        #   cv2.waitKey(1)
        #for b in range(batch_size):
        #    opt_img = opt.data[b].cpu().numpy().transpose(1, 2, 0)
        #    cv2.imshow("opt", opt_img)
        #    cv2.waitKey(0)
        ...
        # style:real ;content:fake 
        #   style Gram Matrix Losses
        #out_fake = vgg(fakeImageBatch_original, style_layers)
        #style_real_content_fake_losses = [style_weights[a] * loss_fns_style_only[a](A, style_targets[a]) for a,A in enumerate(out_fake)]
        
        #nst_loss_1 = sum(layer_losses) * alpha_nst_1
        #nst_loss_real_fake_style = sum(style_real_content_fake_losses) * alpha_nst_style
        #nst_loss_1.backward()
        #nst_loss_real_fake_style.backward()
        #if steps % display_per_iters == 0:
        #    print('\n\n====Style: Real Content: GAN NST Style Gram Matrix Loss is {:6.2f} style + content loss is {:6.2f}=====\n\n'.format(nst_loss_real_fake_style.item(), nst_loss_1.item()))
        #logger.info('\n\n====Style: Real Content: GAN NST Style Gram Matrix Loss is {:6.2f} style + content loss is {:6.2f}=====\n\n'.format(nst_loss_real_fake_style.item(), nst_loss_1.item()))
        ...
        toKeep_discriminator = predictionsFake_discriminator < discriminator_fake_thresh
        toKeep_discriminator = toKeep_discriminator.data.cpu().numpy()
        if steps % display_per_iters == 0:
           print("To Keep Discriminator")
           print(toKeep_discriminator)
        toKeep_final = toKeep_discriminator
        for i in range(toKeep_discriminator.shape[0]):
            if toKeep_discriminator[i] == True and toKeep[i] == True: #mostlikely prob > threshold and netD returns this is a real image not fake
               toKeep_final[i] = True
            else:
               toKeep_final[i] = False
        if steps % display_per_iters == 0:
           print("Final To Keep")
           print(toKeep_final) 
        #print(sum(toKeep))
        if sum(toKeep_final) != 0:
            fakeClassifierLoss = criterion(predictionsFake[toKeep_final], predictedLabels[toKeep_final])  * alpha_c * advWeight
        ...
        if steps % display_per_iters == 0:  
            print('Epoch {:3d} Step {:5d}/{:5d} L_C is {:6.2f} aKL is {:4.2f}  mmd is {:6.2f} Tot Trained {:7d} '.format(epoch, steps, epoch_imgs // batch_size, realClassifierLoss.item(), avg_kl.item(), cur_mmd.item(), total_trained_samples)) 
        logger.info('Epoch {:3d} Step {:5d}/{:5d} L_C is {:6.2f} aKL is {:4.2f}  mmd is {:6.2f} Tot Trained {:7d} '.format(epoch, steps, epoch_imgs // batch_size, realClassifierLoss.item(), avg_kl.item(), cur_mmd.item(), total_trained_samples))
...
if resume_training:
    ...
    #  (but use latest discrimiantor and generator)
    # load model do not load netC
