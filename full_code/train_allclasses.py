...
show_test_classifier_acc_per_iters = 300 # 
...
def train(total_trained_samples):
  ...
  test_acc, best_acc, best_epoch, best_per_class_acc = test_all(-1, best_acc, best_epoch, best_per_class_acc)
    ...
    for steps in range(epoch_imgs // batch_size):
        ...
        if sum(toKeep) != 0:
            ...
            for l in range(real_num):
                ...
                inputs2_cat = torch.cat((inputs2, inputs2, inputs2), dim = 1)
             
                predictions2 = netC(inputs2_cat)
        ...
        if steps % show_test_classifier_acc_per_iters == 0:
            test_acc, best_acc, best_epoch, best_per_class_acc = test_all(epoch, best_acc, best_epoch, best_per_class_acc)
            print("Test acc {:6.2f} Best test acc {:6.2f} best epoch {:5d}".format(test_acc, best_acc, best_epoch))
        ...
        if (steps * batch_size) % save_per_samples == 0:
            model_file = '../models_allclasses/' + model_ckpt_prefix + 'bak_' + str(epoch + 1) + '_' + str((steps * batch_size) // save_per_samples) + '.pth'
            torch.save(state, model_file)

    model_file = '../models_allclasses/' + model_ckpt_prefix + 'epo_' + str(epoch + 1) + '.pth'
    torch.save(state, model_file)
...
start_epoch = 0
if resume_training:
	start_epoch, total_trained_samples = load_model('../models_allclasses/ecgan-chest-xray14epo_100.pth')
    
    #load_gan_and_vanilla('../models_apr28/ecgan-chest-xray14epo_98.pth', '../models_vanilla/ecgan-chest-xray14epo_101.pth')
