...
alpha_mmd = 3
alpha_c = 1.0
alpha_nst_1 = 0.001 #0.003 first for ~3 epochs (at first around 300)
alpha_nst_style = 0.015
confidenceThresh = 0.75
train_g_more = True
train_d_more = not(train_g_more)
g_vs_d = 3 # 3 set to 1 -> 3 (loss_d is very small)
d_vs_g = 3
per_class_sample = True
weights_prefix = '../models_allclasses_wis_mmd_nst/'
weights_middle_name = "_someclasses_is_mmd_nst_thresh0.75_may7_epo_"
#0. torch imports
import torch
...
# True useful labels
usable_label_arr = np.ones(n_classes)
usable_label_arr[4] = 1
usable_label_arr[9] = 1
usable_label_arr[13] = 1

# May 6: extend to all classes starting from 127
for i in range(n_classes):
    usable_label_arr[i] = 1
        ...
        if per_class_sample:
            ...
            for cla in range(n_classes):
                if usable_label_arr[cla] == 0:                
                    continue
                if len(train_val_per_label_list[cla]) == 0:
                    continue
...
def test_all(file, epoch, best_accuracy, best_epoch, best_per_class_acc):
    ...
    for cla in range(n_classes):
        if total_test_per_class[cla] != 0:
           ...
           logger.info("Class {:4d} acc {:6.2f} all {:5d}".format(cla, 100 * correct_test_per_class[cla] / total_test_per_class[cla], total_test_per_class[cla]))
...
start_epoch = 0
if resume_training:
	start_epoch, total_trained_samples = load_model('../models_allclasses_wis_mmd_nst/ecgan-chest-xray14_allclasses_is_mmd_nst_thresh0.75_may6_realall_epo_160.pth') #('../models_allclasses_wis_mmd_nst/ecgan-chest-xray14_allclasses_is_mmd_epo_127.pth')
